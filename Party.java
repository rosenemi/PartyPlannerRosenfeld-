...
/* This method assigns each person a table and seat. If the data is in order by company no two ppl from the same company will be sat at the same table */
  public void Sort(){
    ...
    while (i < numP){
      ...
      ppl[i].table = tId + 1; //updates the persons table number raned corrects for the indexing error
      ppl[i].seat = sId + 1; // same as above but with eat 
      i++;
    }
  }
  ...
  public void PrintComp(){
    ...
    for (int x = 0; x < numP; x++){
      if (ppl[x].companyNum == fComp){
        System.out.println(ppl[x].fName + " " + (ppl[x].table + 1) + " " + ppl[x].seat);
      } // closes if
    } // closes for
